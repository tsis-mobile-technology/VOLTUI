var RealGrids = (function() {
	...
	var win = window;
	var doc = document;
	var REQUIRED_VERSION = "1.1.2.2112";
	var TRACE = false;
	...
	DataProvider.prototype = {
		...
		getRowData: function (row) {
			return this.dom.getRowData(row);
		},
		// @Deprecated use getRowData() instead.
		getRow: function (row) {
			return this.dom.getRowData(row);
		},
		// @Deprecated use getRowData() instead.
	...
	var TreeDataProvider = function () {
		...
		this.onRowAdding = function (provider, rowId) {
			TRACE && _log("onRowAdding:" + rowId);
			return true;
		},
		this.onRowAdded = function (provider, rowId) {
			TRACE && _log("onRowAdded:" + rowId);
		},
		this.onRowsAdded = function (provider, rowIds) {
			TRACE && _log("onRowsAdded:" + rowIds.length);
		},
	...
	GridBase.prototype = {
		...
		setRowData: function (itemIndex, values) {
			return this.dom.setValues(itemIndex, values);
		},
		// return 되는 row 개체의 __rowId 속성에 row id가 담겨있다.
		getValues: function (itemIndex) {
			return this.dom.getValues(itemIndex);
		},
		setValues: function (itemIndex, values) {
			return this.dom.setValues(itemIndex, values);
		},
		...
		getColumnProperty: function (column, prop) {
			if (typeof column === "string") {
				this.dom.setColumnProperty(col.refId, prop, value);
				return col && this.dom.getColumnProperty(col.refId, prop);
			} else if (column) {
				return this.dom.getColumnProperty(column.refId, prop);
			}
		},		
		setColumnProperty: function (column, prop, value) {
			if (typeof column === "string") {
				...
				this.dom.setColumnProperty(column.refId, prop, value);
			}
		},		
		/// column layout
		...
		setCellStyle: function (dataRow, field, styleId) {
			this.dom.setCellStyles(dataRow, field, styleId);
		},
	...
	GridView.prototype = _extend(new GridBase(null, true), {
		...
		getCheckedRows: function (sort) {
			sort = arguments.length > 0 ? sort : true;
			return this.dom.getCheckedRows(sort);
		},
		...
		checkAll: function (checked) {
			...
			return this.dom.checkAll(checked);
		},
		...
		// paging
		setPaging: function (paging, pageSize, pageCount) {
		...
		setPage: function (page, startRow) {
	...
	TreeView.prototype = _extend(new GridBase(null, true), {
		...
		getAncestors: function (itemIndex) {
			return this.dom.getDescendants(itemIndex);
		},	
		...
		isCheckedRow: function (rowId) {
			return this.dom.isCheckedRow(rowId);
		},
		checkItem: function (itemIndex, checked, exclusive) {
			checked = arguments.length > 1 ? checked : true;
			exclusive = arguments.length > 2 ? exclusive : false;
		...
		checkRow: function (rowId, checked, exclusive) {
			checked = arguments.length > 1 ? checked : true;
			exclusive = arguments.length > 2 ? exclusive : false;
			this.dom.checkRow(rowId, checked, exclusive);
		},
		checkAll: function (checked) {
			checked = arguments.length > 0 ? checked : true;
			return this.dom.checkAll(checked);
		...
		checkRows: function (rowIds, checked) {
			checked = arguments.length > 1 ? checked : true;
			this.dom.checkRows(rowIds, checked);
		},
	...
	return {
		...
		onDataRowMoving: function (id, row, newRow) {
			...
			if (grid && grid.dataProvider && grid.dataProvider.onRowMoving) {
				return grid.dataProvider.onRowMoving(grid.dataProvider, row);
			}
		},
		onDataRowMoved: function (id, row, newRow) {
			var grid = __grids[id];
			if (grid && grid.dataProvider && grid.dataProvider.onRowMoved) {
				grid.dataProvider.onRowMoved(grid.dataProvider, row);
			}
		},
		...
		onDataRowStatesCleared: function (id) {
			var grid = __grids[id];
			if (grid && grid.dataProvider && grid.dataProvider.onDataRowStatesCleared) {
				grid.dataProvider.onDataRowStatesCleared(grid.dataProvider);
			}
		},
		...
		onTreeDataRowAdding: function (id, rowId) {
			var tree = __grids[id];
			if (tree && tree.dataProvider && tree.dataProvider.onRowAdding) {
				return tree.dataProvider.onRowAdding(tree.dataProvider, rowId);
			}
		},
		...
		onTreeDataRowsAdded: function (id, rowIds) {
			var tree = __grids[id];
			if (tree && tree.dataProvider && tree.dataProvider.onRowsAdded) {
				tree.dataProvider.onRowsAdded(tree.dataProvider, rowIds);
			}
		},
		onTreeDataRowRemoved: function (id, rowId) {
			var tree = __grids[id];
			if (tree && tree.dataProvider && tree.dataProvider.onRowRemoved) {
				tree.dataProvider.onRowRemoved(tree.dataProvider, rowId);
			}
		},
		onTreeDataRowsRemoved: function (id, rowIds) {
			var tree = __grids[id];
			if (tree && tree.dataProvider && tree.dataProvider.onRowsRemoved) {
